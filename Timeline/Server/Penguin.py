...
class WebSocketMixin(Protocol):
    ...
    def sendLine(self, buffer):
        print self.sendMessage(buffer)

    def __del__(self):
        self.logger.warn('Discarding WebSocketMixin')
...
class Penguin(WebSocketMixin, PenguinDB, ExtensibleObject):
    '''
